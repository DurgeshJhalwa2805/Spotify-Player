...
const AudioControls = () => {
  ...
  return (
    ...
    {/* seekbar */}
      <div
        ref={seekBg}
        className="w-[100%] audio-controller bg-gray-700 rounded-full cursor-pointer mt-3 mb-3 md:mb-2"
        onClick={seekSong}
      >
      ...
      <div className="audio-controller mt-4 flex justify-between items-center">
        <div className="rounded-full p-2  cursor-pointer backdrop-blur-2xl  hover:brightness-200 transition-all duration-300 ease-in-out">
          <img
            src={optionIcon}
            alt={"option"}
            className="h-7 w-7 md:h-4 md:w-4 2xl:h-5 2xl:w-5 rounded-full"
          />
        </div>
          ...
          >
            {playStatus ? (
              <img
                src={pauseIcon}
                alt={"pause"}
                className="h-12 w-12 md:h-8 md:w-8 2xl:h-10 2xl:w-10 rounded-full shadow-lg border-1 border-gray-900"
              />
            ) : (
              <img
                src={playIcon}
